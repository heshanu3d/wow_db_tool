...
- 职业配置页按 GCD、持续时间、伤害/治疗、天赋、触发几率等修改类型拆分；每个技能都可通过复选框决定是否执行，并可在输入框中修改数值、倍率或时长。表头与技能行使用同一套固定列网格，启用、技能、修改值、当前值和技能描述会保持对齐。
...
- “管理自定义技能”可在 GUI 中新增、编辑或删除不在职业源码 `cond` 中的技能：选择修改类型、填写技能定义名称、修改值和 `WHERE` 后的查询条件即可。查询条件示例：`s.SpellName4='死亡之握' and s.ID=49576`。
