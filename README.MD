...
- 职业配置页按 GCD、持续时间、伤害/治疗、天赋、触发几率等修改类型拆分；每个技能都可通过复选框决定是否执行，并可在输入框中修改数值、倍率或时长。“当前值”列会根据修改类型动态读取实际会被 SQL 修改的字段（例如 GCD、持续时间、效果点数、触发几率、冷却时间和施法时间），多等级技能显示数值范围；最右侧从当前选择的 `spell` 表读取 `SpellDescription4`（为空时读取 `SpellToolTip4`）并显示技能描述。当前数据库没有匹配记录时会明确标记为未找到。
