...
- 职业配置页中，每个技能都可通过复选框决定是否执行，并可在输入框中修改数值、倍率或时长。表头与技能行使用同一套固定列网格，启用、技能、修改值、当前值和技能描述会保持对齐。页面支持 Windows/macOS 的鼠标滚轮事件以及 Linux/X11 的滚轮按钮事件。
...
- “管理自定义技能”可在 GUI 中新增、编辑或删除不在职业源码 `cond` 中的技能：选择修改类型、填写技能定义名称、修改值和 `WHERE` 后的查询条件即可。编辑器根据现有职业 `cond` 汇总了名称、等级、单个/多个 ID、ID 范围、职业法术家族、描述和效果字段等 13 个查询模板；选择模板并点击“套用模板”后，可继续在查询条件中手动微调。
