...
- 技能输入框默认读取对应职业源码中的 `mod_*_skills` 字典；“恢复代码默认值”可随时恢复当前源码值，并清除尚未保存的 GUI 自定义技能。
...
数据库连接、职业技能配置和已调整的对话框尺寸/位置都保存在项目根目录的 `.db_tool_gui.json`。技能配置点击“保存配置”后持久化，窗口布局在关闭对应对话框时持久化，重新启动 GUI 仍会保留。该文件可能包含数据库密码，已加入 `.gitignore`，请不要手动提交。
